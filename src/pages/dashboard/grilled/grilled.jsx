...
import GrilledDetailsCard from "./grilledDetailsCard";
...
const Grilled = () => {
    const [grilledData, setGrilledData] = useState([]);
        ...
        const fetchData = async () => {
            try {
                ...
                if (Array.isArray(data)) {
                    ...
                    const grilledCategories = data.filter(dish => dish.category && dish.category.toLowerCase().includes('grilled and roasted foods'));
                    setGrilledData(grilledCategories);
                } else {
    ...
    const currentItems = grilledData.slice(startIndex, endIndex);

    // Calculate total number of pages
    const totalPages = Math.ceil(grilledData.length / ITEMS_PER_PAGE);
    ...
    return (
        <div>
           <div className="grid grid-cols-3 gap-x- gap-y-24">
            ...
            <div className="flex justify-center  items-center mt-32">
                ...
                <button 
                    onClick={nextPage} 
                    disabled={endIndex >= grilledData.length} 
