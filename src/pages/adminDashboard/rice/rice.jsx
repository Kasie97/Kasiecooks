...
import RiceDetailsCard from "./riceDetailsCard";
...
const Rice = () => {
        ...
        const fetchData = async () => {
          try {
            ...
            if (Array.isArray(data)) {
              ...
              const riceCategories = data.filter(dish => dish.category && dish.category.toLowerCase().includes('rice dishes'));
              
              if (riceCategories.length > 0) {
                setRiceData(riceCategories);
                console.log(riceCategories);
              } else {
                console.log('No items found in the "Rice Dishes" category.');
              }
            } else {
    ...
    const removeItemFromBasket = (id) => {
       ...
       const currentItems = riceData.slice(startIndex, endIndex);
   
       // Calculate total number of pages
       const totalPages = Math.ceil(riceData.length / ITEMS_PER_PAGE);
    ...
    return (
        ...
        <div className="flex justify-center  items-center mt-32">
            ...
            <button 
                onClick={nextPage} 
                disabled={endIndex >= riceData.length} 
